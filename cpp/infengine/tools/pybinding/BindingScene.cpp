...
void RegisterSceneBindings(py::module_ &m)
{
        ...
        .def(
            ...
            [](Transform *t, const glm::vec3 &euler, int space) {
                if (space == static_cast<int>(CoordinateSpace::Self)) {
                    ...
                    glm::quat deltaRot = glm::quat(glm::radians(euler));
    ...
    py::class_<MeshRenderer, Component>(m, "MeshRenderer")
        ...
        .def("has_render_material", &MeshRenderer::HasRenderMaterial, "Check if a custom material is assigned")
        .def_property("render_material", &MeshRenderer::GetRenderMaterial, &MeshRenderer::SetRenderMaterial,
                      "The material used for rendering")
        .def("get_effective_material", &MeshRenderer::GetEffectiveMaterial,
        ...
        .def_property_readonly(
            "vertex_count",
            [](const MeshRenderer &mr) -> size_t { return mr.HasInlineMesh() ? mr.GetInlineVertices().size() : 0; },
            "Number of vertices in the inline mesh (0 if using resource mesh)")
        .def_property_readonly(
            "index_count",
            [](const MeshRenderer &mr) -> size_t { return mr.HasInlineMesh() ? mr.GetInlineIndices().size() : 0; },
            "Number of indices in the inline mesh (0 if using resource mesh)")
        .def(
            "get_positions",
    ...
    py::class_<Camera, Component>(m, "Camera")
        ...
        .def(
            ...
            "Returns (origin_vec3f, direction_vec3f) — origin at near plane and normalised direction.")
        // Serialization
    ...
    registry.Register("PyComponentProxy", [](Component *c) -> py::object {
        return py::cast(dynamic_cast<PyComponentProxy *>(c), py::return_value_policy::reference);
    });
    ...
    py::class_<GameObject>(m, "GameObject")
        ...
        .def(
            ...
            [](GameObject *obj, py::object componentType) -> py::object {
                ...
                if (py::isinstance<py::str>(componentType)) {
                    typeName = componentType.cast<std::string>();
                } else if (py::hasattr(componentType, "_cpp_type_name")) {
                    // BuiltinComponent subclass — use the registered C++ type
                    // name so that user subclasses (e.g. class SunLight(Light))
                    // still create the correct C++ component.
                    std::string cppName = py::str(componentType.attr("_cpp_type_name"));
                    if (!cppName.empty()) {
                        typeName = cppName;
                    } else if (py::hasattr(componentType, "__name__")) {
                        typeName = py::str(componentType.attr("__name__"));
                    }
                } else if (py::hasattr(componentType, "__name__")) {
                    typeName = py::str(componentType.attr("__name__"));
                }
        ...
        .def(
            ...
            py::arg("component_instance"), "Add a Python InfComponent instance to this GameObject")
        .def(
            "get_py_component",
            [](GameObject *obj, py::object componentType) -> py::object {
            ...
            [](GameObject *obj, py::object componentType, bool includeInactive) -> py::object {
                ...
                if (py::isinstance<py::str>(componentType)) {
                    ...
                    if (!cppName.empty()) {
                        ...
                        typeName = py::str(componentType.attr("__name__")).cast<std::string>();
                    }
                } else if (py::hasattr(componentType, "__name__")) {
                    typeName = py::str(componentType.attr("__name__")).cast<std::string>();
                }
