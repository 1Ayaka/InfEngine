 ...
 * @brief Python bindings for glm::vec2 as "vec2f".
 *
                            ...
                            float maxSpeed, float deltaTime)
{
    ...
    glm::vec2 targetPos = current - diff;
    float omega = 2.f / smoothTime;
    float x = omega * deltaTime;
    float exp = 1.f / (1.f + x + 0.48f * x * x + 0.235f * x * x * x);
    glm::vec2 temp = (diff * omega) + currentVelocity;
    glm::vec2 change = temp * deltaTime;
...
{
    using Vec = glm::vec2;
    py::class_<Vec>(m, "vec2f")
        .def(py::init<>())
        .def(py::init<float, float>(), "Construct vec2")
        .def("__getitem__",
        ...
        .def_static("smooth_damp", [](const Vec &current, const Vec &target, Vec &currentVelocity, float smoothTime,
                                      float maxSpeed, float deltaTime) {
            return vec2_util::SmoothDamp(current, target, currentVelocity, smoothTime, maxSpeed, deltaTime);
        });
}
