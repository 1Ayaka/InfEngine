 ...
 * @brief Python bindings for glm::vec3 as "vec3f".
 *
                            ...
                            float maxSpeed, float deltaTime)
{
    ...
    if (dist > maxSpeed * smoothTime)
        diff = diff * (maxSpeed * smoothTime / dist);
    glm::vec3 targetPos = current - diff;
    float omega = 2.f / smoothTime;
    float x = omega * deltaTime;
    float exp = 1.f / (1.f + x + 0.48f * x * x + 0.235f * x * x * x);
    glm::vec3 temp = (diff * omega) + currentVelocity;
    glm::vec3 change = temp * deltaTime;
    currentVelocity = (currentVelocity - temp * omega) * exp;
    return targetPos + (diff + change) * exp;
}
...
{
    using Vec = glm::vec3;
    py::class_<Vec>(m, "vec3f")
        .def(py::init<>())
        .def(py::init<float, float, float>(), "Construct vec3")
        .def("__getitem__",
        ...
        .def_static("ortho_normalize", [](Vec &v1, Vec &v2, Vec &v3) { vec3_util::OrthoNormalize(v1, v2, v3); })
        ...
        .def_static("rotate_towards", &vec3_util::RotateTowards)
        ...
        .def_static("smooth_damp", [](const Vec &current, const Vec &target, Vec &currentVelocity, float smoothTime,
                                      float maxSpeed, float deltaTime) {
            return vec3_util::SmoothDamp(current, target, currentVelocity, smoothTime, maxSpeed, deltaTime);
        });
}
