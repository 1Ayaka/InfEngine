...
void RegisterGUIBindings(py::module_ &m)
{
    py::class_<InfGUIContext>(m, "InfGUIContext")
        ...
        .def(
            ...
            [](InfGUIContext &ctx, const std::string &type) -> py::object {
                // Try uint64_t first
                uint64_t data_int = 0;
        ...
        .def("draw_image_rect", &InfGUIContext::DrawImageRect, py::arg("texture_id"), py::arg("min_x"),
             ...
             py::arg("tint_b") = 1.0f, py::arg("tint_a") = 1.0f,
             "Draw an image quad in absolute screen coordinates on current window draw list")
        ...
        .def(
            ...
            [](InfGUIContext &ctx, const std::string &text,
               float fontSize) -> py::
                                   tuple {
                                       auto [w, h] = ctx.CalcTextSizeA(text, fontSize);
                                       return py::make_tuple(py::float_(w), py::float_(h));
                                   },
            py::arg("text"), py::arg("font_size") = 0.0f,
