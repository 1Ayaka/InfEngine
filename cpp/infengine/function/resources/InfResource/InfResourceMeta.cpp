...
// ----------------------------------
// InfResourceMeta Implementation
// ----------------------------------
void InfResourceMeta::Init(const char *content, size_t contentSize, const std::string &filePath, ResourceType type)
{
    ...
    try {
        ...
        INFLOG_WARN("Failed to get file time: ", e.what());
        auto now = std::chrono::system_clock::now();
        auto time_t = std::chrono::system_clock::to_time_t(now);
        modTimeStr = std::to_string(time_t);
    }
...
void InfResourceMeta::AddMetadata(const std::string &key, const std::any &value)
{
    m_metadata[key] = std::make_pair(InfTypeRegistry::GetInstance().GetTypeName(value.type()), value);
}

const std::string &InfResourceMeta::GetResourceName() const
{
...
const std::string &InfResourceMeta::GetHashCode() const
{
    static const std::string empty;
...
const std::string &InfResourceMeta::GetGuid() const
{
    static const std::string empty;
...
bool InfResourceMeta::HasKey(const std::string &key) const
{
    return m_metadata.find(key) != m_metadata.end();
}

void InfResourceMeta::UpdateFilePath(const std::string &newFilePath)
{
    ...
    try {
        if (std::filesystem::exists(newFilePath)) {
            auto fileTime = std::filesystem::last_write_time(newFilePath);
            auto sctp = std::chrono::time_point_cast<std::chrono::system_clock::duration>(
                fileTime - std::filesystem::file_time_type::clock::now() + std::chrono::system_clock::now());
            auto time_t = std::chrono::system_clock::to_time_t(sctp);
        ...
        INFLOG_WARN("Failed to update modification time: ", e.what());
    }
}

const InfResourceMeta::MetadataMap &InfResourceMeta::GetMetadata() const
{
    return m_metadata;
}

const ResourceType &InfResourceMeta::GetResourceType() const
{
...
std::string InfResourceMeta::GetMetaFilePath(const std::string &resourceFilePath)
{
    return resourceFilePath + ".meta";
}

bool InfResourceMeta::SaveToFile(const std::string &metaFilePath) const
{
    const std::string tempPath = metaFilePath + ".tmp";
    std::ofstream file(tempPath, std::ios::trunc);
    if (!file.is_open()) {
        INFLOG_ERROR("Failed to open meta temp file for writing: ", tempPath);
        return false;
    }
    ...
    try {
        ...
        for (const auto &[key, metaPair] : m_metadata) {
            ...
            } else if (typeName == "enum infengine::ResourceType") {
                entry["value"] = InfTypeRegistry::GetInstance().ToString(typeName, value);
            } else {
                INFLOG_WARN("Unknown type for JSON serialization: ", typeName);
                continue;
            }
            entries[key] = entry;
        }
        ...
        std::error_code ec;
        std::filesystem::rename(tempPath, metaFilePath, ec);
        if (ec) {
            std::filesystem::remove(metaFilePath, ec);
            ec.clear();
            std::filesystem::rename(tempPath, metaFilePath, ec);
        }
        if (ec) {
            INFLOG_ERROR("Failed to finalize meta file: ", metaFilePath, " error: ", ec.message());
            return false;
        }

        INFLOG_DEBUG("Meta file saved: ", metaFilePath);
        return true;
    } catch (const std::exception &e) {
        INFLOG_ERROR("Exception while saving meta file: ", metaFilePath, " - ", e.what());
        return false;
    }
}

bool InfResourceMeta::LoadFromFile(const std::string &metaFilePath)
{
    std::ifstream file(metaFilePath);
    if (!file.is_open()) {
        INFLOG_DEBUG("Meta file not found: ", metaFilePath);
        return false;
    }
    ...
    try {
        ...
        for (auto &[key, entry] : root["metadata"].items()) {
            ...
            if (typeName == "string") {
                ...
                std::any resourceType = InfTypeRegistry::GetInstance().FromString(typeName, valStr);
                AddMetadata(key, std::any_cast<ResourceType>(resourceType));
            } else {
                INFLOG_WARN("Unknown type in JSON meta: ", typeName);
            }
        }
        INFLOG_DEBUG("Meta file loaded: ", metaFilePath);
        return true;
    } catch (const std::exception &e) {
        INFLOG_ERROR("Exception while loading meta file: ", metaFilePath, " - ", e.what());
        return false;
    }
}
