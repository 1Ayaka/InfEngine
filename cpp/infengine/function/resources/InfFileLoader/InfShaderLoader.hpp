...
class InfShaderLoader
{
  public:
    ...
    std::shared_ptr<std::vector<char>> Compile(const char *content, size_t contentSize, InfResourceMeta &metaData);
    ...
    /// Set by Load() when glslang parse/link fails; read by InfEngine::ReloadShader.
    ...
    /// Consumed by InfEngine::ReloadShader to register the shadow variant.
