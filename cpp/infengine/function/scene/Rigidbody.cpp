...
INFENGINE_REGISTER_COMPONENT("Rigidbody", Rigidbody)
...
Rigidbody::~Rigidbody()
{
    ...
    SceneManager::Instance().UnregisterRigidbody(this);
...
{
    SceneManager::Instance().RegisterRigidbody(this);
...
void Rigidbody::ApplyInterpolatedTransform(float alpha)
{
    ...
    d.lastSyncedRotation = presentedRot;
    d.hasSyncedOnce = true;

    auto colliders = go->GetComponents<Collider>();
    for (auto *c : colliders) {
        if (c && c->GetBodyId() != 0xFFFFFFFF) {
            c->SetLastSyncedTransform(presentedPos, presentedRot);
        }
    }
}
...
void Rigidbody::ApplyMotionQuality()
{
    ...
    // Jolt: 0 = Discrete, 1 = LinearCast.
    // Unity modes 1,2,3 all map to Jolt's LinearCast (continuous).
    int joltQuality = (d.collisionDetectionMode > 0) ? 1 : 0;
