...
{
    if (!scene) {
        INFLOG_WARN("CSM: ComputeShadowVP skipped because scene is null");
        return;
    }
    ...
    if (!camera) {
        INFLOG_WARN("CSM: ComputeShadowVP found no camera; using fallback frustum parameters");
    }
    ...
    for (Light *light : activeLights) {
        ...
        if (!loggedShadowLight) {
            ...
            INFLOG_INFO("CSM: '", obj->GetName(), "' forward=(", fwd.x, ",", fwd.y, ",", fwd.z,
    ...
    if (s_noShadowLightWarnCount++ < 8) {
        INFLOG_WARN("CSM: no active directional light with shadows enabled was found in the scene");
    }
}
