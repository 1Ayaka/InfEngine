...
static void JoltTraceImpl(const char *fmt, ...)
{
    ...
    va_end(args);
    INFLOG_DEBUG("[Jolt] ", buffer);
}

#ifdef JPH_ENABLE_ASSERTS
static bool JoltAssertFailed(const char *expression, const char *message, const char *file, unsigned int line)
{
    INFLOG_ERROR("[Jolt Assert] ", file, ":", line, " – ", expression, " – ", message ? message : "");
    // IMPORTANT: returning true here triggers __debugbreak() inside the Jolt
    // assert macro, which raises EXCEPTION_BREAKPOINT and crashes the process
    // when no debugger is attached.  Return false to log-and-continue instead.
    return false;
}
#endif
...
void PhysicsWorld::Initialize()
{
    ...
    INFLOG_INFO("PhysicsWorld: Initializing Jolt Physics…");
    ...
    // -------------------------------------------------------------------------
    static constexpr size_t kTempAllocatorSize =
        256 * 1024 * 1024; // 256 MB — safe for ≥1024 bodies with 2 collision steps

    int hwThreads = static_cast<int>(std::thread::hardware_concurrency());
    int numThreads = std::clamp(hwThreads - 1, 1, 8);

    m_tempAllocator = std::make_unique<JPH::TempAllocatorImpl>(kTempAllocatorSize);

    // JPH::cMaxPhysicsJobs = 2048 (Jolt built-in) is the MINIMUM recommended
    // value.  With 1000+ active dynamic bodies Jolt spawns many concurrent jobs
    // (broadphase update, narrowphase contact pairs, constraint solver islands).
    // Exceeding the slot count fires a Jolt assert that — without a debugger
    // attached — crashes the process via __debugbreak().  4096 jobs and 16
    // barriers keep us well below the limit for scenes up to ~4096 bodies.
    static constexpr uint32_t kMaxPhysicsJobs = 4096;
    static constexpr uint32_t kMaxPhysicsBarriers = 16;
    m_jobSystem = std::make_unique<JPH::JobSystemThreadPool>(kMaxPhysicsJobs, kMaxPhysicsBarriers, numThreads);

    // Layer interfaces
    m_layers = std::make_unique<LayerInterfaces>();

    // Physics system
    //   maxBodies           = 65536  (was 1024 — limits scene scale)
    //   numBodyMutexes      = 0      (auto — Jolt picks power-of-2)
    //   maxBodyPairs        = 65536  (was 1024 — 1024 bodies can easily exceed 1024 pairs)
    //   maxContactConstraints = 65536
    m_physicsSystem = std::make_unique<JPH::PhysicsSystem>();
    m_physicsSystem->Init(65536, 0, 65536, 65536, m_layers->bpInterface, m_layers->objVsBpFilter,
                          m_layers->objPairFilter);

    // Gravity (Unity default: -9.81)
    m_physicsSystem->SetGravity(JPH::Vec3(0.0f, -9.81f, 0.0f));

    // Install contact listener for collision/trigger callbacks
    m_contactListener = std::make_unique<InfContactListener>();
    ...
    m_initialized = true;
    INFLOG_INFO("PhysicsWorld: Jolt Physics initialized.");
}
...
void PhysicsWorld::Shutdown()
{
    ...
    INFLOG_INFO("PhysicsWorld: Shutting down...");
...
uint32_t PhysicsWorld::CreateBody(Collider *collider, bool isStatic, bool isTrigger)
{
    ...
    if (!body) {
        INFLOG_ERROR("PhysicsWorld: Failed to create body.");
        return 0xFFFFFFFF;
    }
...
void PhysicsWorld::SetBodyMotionQuality(uint32_t bodyId, int quality)
{
    ...
    JPH::EMotionQuality mq = (quality == 1) ? JPH::EMotionQuality::LinearCast : JPH::EMotionQuality::Discrete;
    JPH::BodyInterface &bi = m_physicsSystem->GetBodyInterface();
...
std::vector<Collider *> PhysicsWorld::OverlapSphere(const glm::vec3 &center, float radius,
                                                     uint32_t layerMask, bool queryTriggers) const
{
    ...
    for (const auto &hit : collector.mHits) {
        ...
        if (!queryTriggers && IsBodySensor(bodyId))
            continue;
        if (seen.insert(bodyId).second) {
            Collider *col = FindColliderByBodyId(bodyId);
            if (col)
                               ...
                               RaycastHit &outHit, uint32_t layerMask, bool queryTriggers) const
{
    ...
    outHit.normal = glm::vec3(-result.mPenetrationAxis.GetX(),
                               -result.mPenetrationAxis.GetY(),
                               -result.mPenetrationAxis.GetZ());
    float nLen = glm::length(outHit.normal);
    if (nLen > 1e-6f) outHit.normal /= nLen;

    outHit.collider = FindColliderByBodyId(bodyId);
...
Collider *PhysicsWorld::ResolveColliderForSubShape(uint32_t bodyId, uint32_t subShapeIdValue) const
{
    ...
    auto *compound = dynamic_cast<const JPH::CompoundShape *>(shape);
    if (!compound) {
        return fallback;
    }

    uint32_t subShapeCount = compound->GetNumSubShapes();
    uint32_t subShapeBits = 0;
    uint32_t maxIndex = (subShapeCount > 0u) ? (subShapeCount - 1u) : 0u;
    while (maxIndex > 0u) {
        ++subShapeBits;
        maxIndex >>= 1u;
    }
    uint32_t subShapeMask = (subShapeBits >= 32u) ? 0xFFFFFFFFu : ((1u << subShapeBits) - 1u);
    uint32_t subShapeIndex = subShapeIdValue & subShapeMask;
    if (subShapeIndex >= subShapeCount) {
        return fallback;
    }
...
void PhysicsWorld::EnsureSceneBodiesRegistered(Scene *scene)
{
    ...
    // Use the SceneManager physics registry instead of GetAllObjects()
    auto &sm = SceneManager::Instance();
    bool anyRegistered = false;

    for (auto *col : sm.GetActiveColliders()) {
        if (!col || !col->IsEnabled())
            continue;
