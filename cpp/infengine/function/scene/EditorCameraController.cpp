...
void EditorCameraController::ApplyPan(float deltaX, float deltaY)
{
    ...
    // Scale by focus distance to make pan speed feel consistent
    glm::vec3 right = transform->GetRight();
    glm::vec3 up = transform->GetUp();

    // Use smaller multiplier and scale by distance for consistent feel
    float distanceScale = std::max(1.0f, m_focusDistance * 0.1f);
    glm::vec3 pan = (-right * deltaX + up * deltaY) * panSpeed * distanceScale;
