...
Collider::~Collider()
{
    // Unregister from physics registry before cleanup
    SceneManager::Instance().UnregisterCollider(this);
...
{
    SceneManager::Instance().RegisterCollider(this);
    RegisterBody();
...
glm::vec3 Collider::GetWorldScale() const
{
    if (auto *go = GetGameObject()) {
        if (auto *tf = go->GetTransform()) {
            return glm::abs(tf->GetLocalScale());
        }
    }
...
void Collider::RegisterBody()
{
    ...
    for (auto *col : colliders) {
        ...
        if (auto *tf = go->GetTransform()) {
            ...
            other.lastScale = tf->GetLocalScale();
        }
    }
    ...
    if (d.cachedRigidbody && d.cachedRigidbody->IsEnabled() && d.bodyId != 0xFFFFFFFF) {
        ...
        // Collision detection mode: 0 = Discrete, >0 = Continuous (Jolt LinearCast)
        int joltQuality = (d.cachedRigidbody->GetCollisionDetectionMode() > 0) ? 1 : 0;
...
void Collider::SyncTransformToPhysics()
{
    ...
    // Rebuild the Jolt shape when scale changes (shape bakes scale at creation)
    glm::vec3 currentScale = tf->GetLocalScale();
