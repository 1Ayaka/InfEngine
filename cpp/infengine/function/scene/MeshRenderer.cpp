...
void MeshRenderer::SetMeshAsset(const std::string &guid, std::shared_ptr<InfMesh> mesh)
{
    ...
    m_meshAsset = AssetRef<InfMesh>(guid, std::move(mesh));
...
void MeshRenderer::SetMaterial(uint32_t slot, std::shared_ptr<InfMaterial> material)
{
    ...
    ref = AssetRef<InfMaterial>(guid, std::move(material));
...
std::shared_ptr<InfMaterial> MeshRenderer::GetMaterial(uint32_t slot) const
{
    if (slot >= m_materials.size())
        return nullptr;
...
std::shared_ptr<InfMaterial> MeshRenderer::GetEffectiveMaterial(uint32_t slot) const
{
...
void MeshRenderer::UpdateBoundsForNodeGroup(const std::shared_ptr<InfMesh> &mesh)
{
