...
class PrimitiveMeshes
{
  public:
    ...
    {
        static std::vector<Vertex> vertices = CreateSphereVertices(16, 16);
        return vertices;
    }
    ...
    {
        static std::vector<uint32_t> indices = CreateSphereIndices(16, 16);
        return indices;
    }
    ...
    static std::vector<Vertex> CreateSphereVertices(int segments, int rings)
    {
        ...
        for (int ring = 0; ring <= rings; ++ring) {
            ...
            for (int seg = 0; seg <= segments; ++seg) {
                ...
                // Tangent is perpendicular to the normal in the XZ plane
