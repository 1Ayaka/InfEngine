 ...
 *
 * Material reference uses a single AssetRef<InfMaterial> identified by GUID.
 * Serialization writes only "materialGuid"; deserialization resolves via
 * AssetRegistry to obtain the live InfMaterial pointer.
 */
class MeshRenderer : public Component
{
  public:
    ...
    void SetMeshAsset(const std::string &guid, std::shared_ptr<InfMesh> mesh);
    ...
    [[nodiscard]] const AssetRef<InfMesh> &GetMeshAssetRef() const
    {
        return m_meshAsset;
    }
    ...
    [[nodiscard]] std::shared_ptr<InfMaterial> GetMaterial(uint32_t slot = 0) const;

    /// @brief Get the effective material for a slot (returns default if none).
    [[nodiscard]] std::shared_ptr<InfMaterial> GetEffectiveMaterial(uint32_t slot = 0) const;
    ...
    void SetMaterial(uint32_t slot, std::shared_ptr<InfMaterial> material);
    ...
    [[nodiscard]] const std::vector<AssetRef<InfMaterial>> &GetMaterialRefs() const
    {
        return m_materials;
    }
    ...
    void UpdateBoundsForNodeGroup(const std::shared_ptr<InfMesh> &mesh);
  ...
  private:
    ...
    std::vector<AssetRef<InfMaterial>> m_materials;

    // Mesh asset reference (for model-file meshes managed by AssetRegistry)
    AssetRef<InfMesh> m_meshAsset;
