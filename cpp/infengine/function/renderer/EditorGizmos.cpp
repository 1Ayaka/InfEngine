...
void EditorGizmos::GetCombinedGizmoData(std::vector<Vertex> &outVertices, std::vector<uint32_t> &outIndices)
{
    outVertices.clear();
    outIndices.clear();

    // Add grid only (outline is rendered in a separate pass with its own shader)
    if (m_showGrid) {
        const auto &gridVerts = GetGridVertices();
        const auto &gridInds = GetGridIndices();

        size_t baseIndex = outVertices.size();
        outVertices.insert(outVertices.end(), gridVerts.begin(), gridVerts.end());

        for (uint32_t idx : gridInds) {
            outIndices.push_back(static_cast<uint32_t>(baseIndex + idx));
        }
    }
}

DrawCallResult EditorGizmos::GetDrawCalls(std::shared_ptr<InfMaterial> gizmoMaterial,
                                          std::shared_ptr<InfMaterial> gridMaterial, uint64_t selectedObjectId,
                                          Scene *activeScene, const glm::vec3 &cameraPos)
{
    ...
    // NOTE: Outline is now rendered via post-process passes in InfVkCoreModular
...
void EditorGizmos::CreateOutlineMesh()
{
    ...
    if (!m_hasSelectionOutline || m_selectionPositions.empty() || m_selectionIndices.empty()) {
        INFLOG_DEBUG("CreateOutlineMesh: no selection data, hasOutline=", m_hasSelectionOutline,
                     ", positions=", m_selectionPositions.size(), ", indices=", m_selectionIndices.size());
        return;
    }

    INFLOG_DEBUG("CreateOutlineMesh: creating outline with ", m_selectionPositions.size(), " positions");
...
void EditorGizmos::CreateGridMesh()
{
    ...
    // Avoids drawing each pixel twice which would compound alpha blending.
