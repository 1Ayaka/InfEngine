...
class EditorGizmos
{
  public:
    ...
    [[nodiscard]] float GetGridSpacing() const
    {
    ...
    /// @brief Get combined gizmo vertices (grid only - outline is rendered separately)
    void GetCombinedGizmoData(std::vector<Vertex> &outVertices, std::vector<uint32_t> &outIndices);
    ...
    [[nodiscard]] DrawCallResult GetDrawCalls(std::shared_ptr<InfMaterial> gizmoMaterial,
  ...
  private:
    ...
    // Unity-style grid: 100x100 units with 1 unit spacing
    float m_gridSize = 100.0f;      // Grid extends from -size to +size (200x200 total)
    float m_gridSpacing = 1.0f;     // Distance between grid lines
    float m_majorLineEvery = 10.0f; // Major (thicker) lines every N units
