...
RenderPassOutput::RenderPassOutput(InfVkCoreModular *vkCore) : m_vkCore(vkCore)
{
}
...
{
    if (!m_vkCore) {
        INFLOG_ERROR("RenderPassOutput::Initialize: vkCore is null");
        return false;
    }
    ...
    try {
        ...
        INFLOG_INFO("RenderPassOutput '", name, "' initialized: ", width, "x", height);
        return true;
    } catch (const std::exception &e) {
        INFLOG_ERROR("RenderPassOutput::Initialize failed: ", e.what());
        CleanupResources();
        return false;
    }
}
...
void RenderPassOutput::Resize(uint32_t width, uint32_t height)
{
    ...
    try {
        ...
        m_isInitialized = true;
        INFLOG_DEBUG("RenderPassOutput '", m_name, "' resized: ", width, "x", height);
    } catch (const std::exception &e) {
        INFLOG_ERROR("RenderPassOutput::Resize failed: ", e.what());
...
bool RenderPassOutput::ReadbackColorPixels(std::vector<uint8_t> &outData)
{
    if (!m_isInitialized || !m_colorStagingBuffer) {
        INFLOG_ERROR("RenderPassOutput::ReadbackColorPixels: not initialized");
        return false;
    }
    ...
    if (vmaMapMemory(allocator, m_colorStagingAllocation, &data) != VK_SUCCESS) {
        INFLOG_ERROR("RenderPassOutput::ReadbackColorPixels: failed to map staging memory");
        return false;
    }
...
bool RenderPassOutput::ReadbackDepthPixels(std::vector<float> &outData)
{
    if (!m_isInitialized || !m_depthStagingBuffer || !m_includeDepth) {
        INFLOG_ERROR("RenderPassOutput::ReadbackDepthPixels: not initialized or no depth");
        return false;
    }

    // Similar implementation for depth buffer readback
    // TODO: Implement depth readback
    INFLOG_WARN("RenderPassOutput::ReadbackDepthPixels: not yet implemented");
    return false;
}
