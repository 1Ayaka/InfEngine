...
void InfVkCoreModular::InvalidateTextureCache(const std::string &textureIdentifier)
{
    ...
    INFLOG_INFO("Invalidating texture cache for: ", matchKey);
...
void InfVkCoreModular::RemoveMaterialPipeline(const std::string &materialName)
{
    if (m_materialPipelineManagerInitialized) {
        m_deviceContext.WaitIdle();
        m_materialPipelineManager.RemoveRenderData(materialName);
        INFLOG_INFO("Removed material pipeline render data for: ", materialName);
    }
}
...
void InfVkCoreModular::FlushDeletionQueue()
{
    m_deletionQueue.FlushAll();
}
