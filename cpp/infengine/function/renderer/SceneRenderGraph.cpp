...
bool ValidatePythonGraphDescription(const RenderGraphDescription &desc)
{
    ...
    for (const auto &tex : desc.textures) {
        if (tex.name.empty()) {
            INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: texture name cannot be empty");
            return false;
        }
        if (!textures.emplace(tex.name, &tex).second) {
            INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: duplicate texture '", tex.name, "'");
            return false;
        }
        if (tex.sizeDivisor == 1) {
            INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: texture '", tex.name,
    ...
    for (const auto &pass : desc.passes) {
        if (pass.name.empty()) {
            INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: pass name cannot be empty");
            return false;
        }
        if (!passNames.insert(pass.name).second) {
            INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: duplicate pass '", pass.name, "'");
            return false;
        }
        if (pass.action == GraphPassActionType::Compute &&
            (pass.clearColor || pass.clearDepth || !pass.writeDepth.empty())) {
            INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: compute pass '", pass.name,
        ...
        if (pass.action == GraphPassActionType::DrawShadowCasters) {
            if (!pass.writeColors.empty()) {
                INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: shadow pass '", pass.name,
        ...
        if (pass.clearDepth && pass.writeDepth.empty()) {
            INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: pass '", pass.name,
        ...
        for (const auto &[slot, textureName] : pass.writeColors) {
            ...
            if (texIt == textures.end()) {
                INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: pass '", pass.name, "' writes unknown color target '",
                             textureName, "'");
                return false;
            }
            if (texIt->second->isDepth) {
                INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: pass '", pass.name, "' writes depth texture '",
        ...
        if (!pass.writeDepth.empty()) {
            ...
            if (texIt == textures.end()) {
                INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: pass '", pass.name, "' writes unknown depth target '",
                             pass.writeDepth, "'");
                return false;
            }
            if (!texIt->second->isDepth) {
                INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: pass '", pass.name, "' writes color texture '",
        ...
        for (const auto &textureName : pass.readTextures) {
            if (textures.find(textureName) == textures.end()) {
                INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: pass '", pass.name, "' reads unknown texture '",
                             textureName, "'");
                return false;
            }
        }
        ...
        for (const auto &[samplerName, textureName] : pass.inputBindings) {
            if (textures.find(textureName) == textures.end()) {
                INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: pass '", pass.name, "' input '", samplerName,
    ...
    if (!desc.outputTexture.empty() && textures.find(desc.outputTexture) == textures.end()) {
        INFLOG_ERROR("SceneRenderGraph::ApplyPythonGraph: output texture '", desc.outputTexture, "' is not declared");
        return false;
    }
...
bool SceneRenderGraph::Initialize(InfVkCoreModular *vkCore, SceneRenderTarget *sceneTarget)
{
    ...
    for (uint32_t i = 0; i < kMaxFramesInFlight; ++i) {
        ...
        if (m_perViewDescSets[i] == VK_NULL_HANDLE) {
            INFLOG_WARN("SceneRenderGraph: Failed to allocate per-view descriptor set [", i, "]");
        }
    }
...
void SceneRenderGraph::EnsureGraphBuilt()
{
    ...
    // InfRenderer::DrawFrame() will detect the mismatch on the NEXT frame
...
void SceneRenderGraph::RefreshPerViewShadowDescriptor()
{
    ...
    if (!m_shadowMapInputHandle.IsValid() || !m_renderGraph) {
        ...
        if (s_missingShadowInputWarnCount++ < 8) {
            INFLOG_WARN("SceneRenderGraph: no valid shadowMap input handle for per-view descriptor; binding fallback "
                        "white texture");
        }
            ...
            if (passDesc.action == GraphPassActionType::FullscreenQuad) {
                ...
                for (const auto &[name, value] : passDesc.pushConstants) {
                    if (packedPushConstantSize / sizeof(float) < 32) {
                        ...
                        INFLOG_ERROR("FullscreenQuad '", shaderName,
                ...
                if (!passDesc.inputBindings.empty()) {
                    ...
                    // Legacy path: use readTextures order (colorReadHandles + backbuffer)
                ...
                m_renderGraph->AddPass(passDesc.name, [=](vk::PassBuilder &builder) {
                    ...
                    return [=, cachedRenderPass =
                        ...
                        if (descSet == VK_NULL_HANDLE) {
                            INFLOG_ERROR("FullscreenQuad '", shaderName, "': descriptor pool exhausted, skipping pass");
                            return;
                        }
