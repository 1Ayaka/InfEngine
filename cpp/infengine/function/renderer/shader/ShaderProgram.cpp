                           ...
                           const std::string &shaderId)
{
    ...
    if (m_vertModule == VK_NULL_HANDLE) {
        INFLOG_ERROR("Failed to create vertex shader module for program: ", shaderId);
        return false;
    }

    m_fragModule = CreateShaderModule(fragSpirv);
    if (m_fragModule == VK_NULL_HANDLE) {
        INFLOG_ERROR("Failed to create fragment shader module for program: ", shaderId);
    ...
    if (!m_vertReflection.Reflect(vertSpirv, VK_SHADER_STAGE_VERTEX_BIT)) {
        INFLOG_ERROR("Failed to reflect vertex shader: ", shaderId);
    }

    if (!m_fragReflection.Reflect(fragSpirv, VK_SHADER_STAGE_FRAGMENT_BIT)) {
        INFLOG_ERROR("Failed to reflect fragment shader: ", shaderId);
    }
    ...
    if (!ValidateStageInterface()) {
        INFLOG_ERROR("Shader interface validation failed for program: ", shaderId,
    ...
    if (!CreateDescriptorSetLayouts()) {
        INFLOG_ERROR("Failed to create descriptor set layouts for program: ", shaderId);
        Destroy();
        return false;
    }
    ...
    if (!CreatePipelineLayout()) {
        INFLOG_ERROR("Failed to create pipeline layout for program: ", shaderId);
        Destroy();
        return false;
    }

    INFLOG_INFO("Created shader program: ", shaderId, " with ", m_descriptorBindings.size(), " bindings");
    return true;
}
...
VkShaderModule ShaderProgram::CreateShaderModule(const std::vector<char> &code)
{
    ...
    if (vkCreateShaderModule(m_device, &createInfo, nullptr, &module) != VK_SUCCESS) {
        INFLOG_ERROR("vkCreateShaderModule failed");
        return VK_NULL_HANDLE;
    }
...
void ShaderProgram::MergeReflectionData()
{
    ...
    INFLOG_DEBUG("Merged ", m_descriptorBindings.size(), " descriptor bindings for shader: ", m_shaderId);
    for (const auto &b : m_descriptorBindings) {
        INFLOG_DEBUG("  binding=", b.binding, " set=", b.set, " type=", static_cast<int>(b.type), " name=", b.name);
    }
}
...
bool ShaderProgram::ValidateStageInterface() const
{
    ...
    for (const auto &fragIn : fragInputs) {
        ...
        if (!matchedOutput) {
            INFLOG_ERROR("Shader interface mismatch in '", m_shaderId, "': fragment input '", fragIn.name,
        ...
        if (matchedOutput->format != fragIn.format) {
            INFLOG_ERROR("Shader interface mismatch in '", m_shaderId, "': vertex output '", matchedOutput->name,
...
bool ShaderProgram::CreateDescriptorSetLayouts()
{
    ...
    for (auto &[setIndex, bindings] : setBindings) {
        ...
        if (vkCreateDescriptorSetLayout(m_device, &layoutInfo, nullptr, &layout) != VK_SUCCESS) {
            INFLOG_ERROR("Failed to create descriptor set layout for set ", setIndex);
            return false;
        }

        m_descriptorSetLayouts[setIndex] = layout;
        INFLOG_DEBUG("Created descriptor set layout for set ", setIndex, " with ", bindings.size(), " bindings");
    }
    ...
    if (m_descriptorSetLayouts.empty()) {
        ...
        VkDescriptorSetLayout layout;
        if (vkCreateDescriptorSetLayout(m_device, &layoutInfo, nullptr, &layout) != VK_SUCCESS) {
            INFLOG_ERROR("Failed to create empty descriptor set layout");
            return false;
        }
...
bool ShaderProgram::CreatePipelineLayout()
{
    ...
    for (uint32_t i = 0; i <= maxSet; ++i) {
        ...
        if (it != m_descriptorSetLayouts.end()) {
            ...
            if (vkCreateDescriptorSetLayout(m_device, &layoutInfo, nullptr, &emptyLayout) != VK_SUCCESS) {
                INFLOG_ERROR("Failed to create empty descriptor set layout for gap at set ", i);
                return false;
            }
    ...
    if (vkCreatePipelineLayout(m_device, &pipelineLayoutInfo, nullptr, &m_pipelineLayout) != VK_SUCCESS) {
        INFLOG_ERROR("Failed to create pipeline layout");
        return false;
    }
...
{
    m_device = device;
    INFLOG_INFO("ShaderProgramCache initialized");
}
...
void ShaderProgramCache::Shutdown()
{
    Clear();
    m_device = VK_NULL_HANDLE;
    INFLOG_INFO("ShaderProgramCache shutdown");
}
                                                      ...
                                                      const std::vector<char> &fragSpirv)
{
    ...
    if (!program->Create(m_device, vertSpirv, fragSpirv, shaderId)) {
        INFLOG_ERROR("Failed to create shader program: ", shaderId);
...
void ShaderProgramCache::RemoveProgramsContainingShader(const std::string &shaderName)
{
    ...
    for (const auto &[key, program] : m_programs) {
        ...
        if (pipePos != std::string::npos) {
            ...
            if (extractShaderName(vertPath) == shaderName || extractShaderName(fragPath) == shaderName) {
                ...
                INFLOG_DEBUG("Removing cached shader program: ", key);
            }
        } else if (key == shaderName || extractShaderName(key) == shaderName) {
            // Fallback for simple key format
            toRemove.push_back(key);
            INFLOG_DEBUG("Removing cached shader program (simple key): ", key);
        }
    }
    ...
    INFLOG_INFO("Removed ", toRemove.size(), " shader programs containing shader '", shaderName, "'");
}
