...
VkShaderModule VkPipelineManager::CreateShaderModule(const std::vector<uint32_t> &code)
{
    ...
    if (result != VK_SUCCESS) {
        INFLOG_ERROR("Failed to create shader module: ", VkResultToString(result));
        return VK_NULL_HANDLE;
    }
...
VkRenderPass VkPipelineManager::CreateRenderPass(const RenderPassConfig &config)
{
    ...
    // Color attachment (skip for depth-only passes like shadow maps)
    if (config.hasColor) {
        ...
        colorAttachment.storeOp = config.hasResolve ? VK_ATTACHMENT_STORE_OP_DONT_CARE : VK_ATTACHMENT_STORE_OP_STORE;
        colorAttachment.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
        colorAttachment.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
        // When clearing, UNDEFINED is optimal (contents are discarded anyway).
        // When loading, use COLOR_ATTACHMENT_OPTIMAL so the prior contents survive
        // the initial layout transition (UNDEFINED -> any discards contents).
        colorAttachment.initialLayout =
            config.clearColor ? VK_IMAGE_LAYOUT_UNDEFINED : VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
        colorAttachment.finalLayout =
            config.hasResolve ? VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL : config.colorFinalLayout;
    ...
    // Read-only depth passes only need read access; write passes need both read+write.
    if (config.readOnlyDepth) {
    ...
    if (result != VK_SUCCESS) {
        INFLOG_ERROR("Failed to create render pass: ", VkResultToString(result));
        return VK_NULL_HANDLE;
    }
                                                         ...
                                                         const std::vector<VkPushConstantRange> &pushConstantRanges)
{
    ...
    if (result != VK_SUCCESS) {
        INFLOG_ERROR("Failed to create pipeline layout: ", VkResultToString(result));
        return VK_NULL_HANDLE;
    }
...
PipelineResult VkPipelineManager::CreateGraphicsPipeline(const PipelineConfig &config)
{
    ...
    if (vertModule == VK_NULL_HANDLE || fragModule == VK_NULL_HANDLE) {
        INFLOG_ERROR("Failed to load shader modules");
    ...
    if (vkResult != VK_SUCCESS) {
        INFLOG_ERROR("Failed to create graphics pipeline: ", VkResultToString(vkResult));
...
VkPipelineManager::CreateDescriptorSetLayout(const std::vector<VkDescriptorSetLayoutBinding> &bindings)
{
    ...
    if (result != VK_SUCCESS) {
        INFLOG_ERROR("Failed to create descriptor set layout: ", VkResultToString(result));
        return VK_NULL_HANDLE;
    }
...
std::vector<uint32_t> VkPipelineManager::ReadShaderFile(const std::string &filePath)
{
    ...
    if (!file.is_open()) {
        INFLOG_ERROR("Failed to open shader file: ", filePath);
        return {};
    }

    size_t fileSize = static_cast<size_t>(file.tellg());
    if (fileSize % sizeof(uint32_t) != 0) {
        INFLOG_ERROR("Invalid SPIR-V file size: ", fileSize, " bytes");
        return {};
    }
