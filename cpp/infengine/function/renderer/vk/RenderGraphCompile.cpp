...
void RenderGraph::TopologicalSort()
{
    ...
    if (m_executionOrder.size() != activePasses.size()) {
        INFLOG_ERROR("RenderGraph::TopologicalSort - Cycle detected! Sorted ", m_executionOrder.size(), " of ",
    ...
    for (uint32_t ri = 0; ri < static_cast<uint32_t>(m_resources.size()); ++ri) {
        ...
        if (resource.type == ResourceType::Texture2D || resource.type == ResourceType::DepthStencil) {
            ...
            if (vkCreateImage(device, &imageInfo, nullptr, &resource.allocatedImage) != VK_SUCCESS) {
                INFLOG_ERROR("Failed to create image for resource: ", resource.name);
                return false;
            }
            ...
            if (vkCreateBuffer(device, &bufferInfo, nullptr, &resource.allocatedBuffer) != VK_SUCCESS) {
                INFLOG_ERROR("Failed to create buffer for resource: ", resource.name);
                return false;
            }
            ...
            if (vmaAllocateMemoryForBuffer(allocator, resource.allocatedBuffer, &allocCreateInfo,
                ...
                INFLOG_ERROR("Failed to allocate memory for buffer: ", resource.name);
                return false;
            }
    ...
    for (auto &req : imageAllocRequests) {
        ...
        if (!placed) {
            ...
            if (vmaAllocateMemory(allocator, &req.memReqs, &allocCreateInfo, &allocation, &vmaAllocInfo) !=
                VK_SUCCESS) {
                INFLOG_ERROR("Failed to allocate memory for resource: ", resource.name);
                return false;
            }

            if (vkBindImageMemory(device, resource.allocatedImage, vmaAllocInfo.deviceMemory, 0) != VK_SUCCESS) {
                vmaFreeMemory(allocator, allocation);
                INFLOG_ERROR("Failed to bind image memory for resource: ", resource.name);
                return false;
            }
        ...
        if (vkCreateImageView(device, &viewInfo, nullptr, &resource.allocatedView) != VK_SUCCESS) {
            INFLOG_ERROR("Failed to create image view for resource: ", resource.name);
            return false;
        }
    }
...
bool RenderGraph::CreateVulkanRenderPasses()
{
    ...
    for (auto &pass : m_passes) {
        ...
        // Color final layout — always COLOR_ATTACHMENT_OPTIMAL for scene targets
        // (the backbuffer is the MSAA color target, not the swapchain)
        ...
        if (cacheIt != m_renderPassCache.end()) {
            ...
            if (pass.vulkanRenderPass == VK_NULL_HANDLE) {
                INFLOG_ERROR("Failed to create render pass for: ", pass.name);
                return false;
            }
    ...
    for (auto &pass : m_passes) {
        ...
        if (cacheIt != m_framebufferCache.end() && cacheIt->second.framebuffer != VK_NULL_HANDLE) {
            ...
            if (vkCreateFramebuffer(device, &framebufferInfo, nullptr, &fb) != VK_SUCCESS) {
                INFLOG_ERROR("Failed to create framebuffer for pass: ", pass.name);
                return false;
            }
            pass.framebuffer = fb;
...
void RenderGraph::InsertBarriers(VkCommandBuffer cmdBuffer, uint32_t passIndex)
{
    ...
    auto addBarrier = [&](const ResourceAccess &access, bool isDepthInput = false) {
        ...
        auto stateIt = m_resourceStates.find(access.handle.id);
        VkImageLayout oldLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        VkAccessFlags srcAccessMask = 0;
        VkPipelineStageFlags srcStages = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;

        if (stateIt != m_resourceStates.end()) {
            oldLayout = stateIt->second.layout;
            srcAccessMask = stateIt->second.accessMask;
            srcStages = stateIt->second.stages;
            // Ensure srcStages is never 0
            if (srcStages == 0)
    ...
    //
    // Read accesses: the barrier transitions the image to the read's target
    // layout, so the tracked state must reflect this for subsequent passes.
    // Without this, consecutive reads of the same resource produce barriers
    // whose oldLayout doesn't match the actual (post-barrier) layout.
    for (const auto &read : pass.reads) {
