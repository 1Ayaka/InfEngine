                                                    ...
                                                    void *pUserData)
{
    ...
    if (messageSeverity >= VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) {
        INFLOG_ERROR("Vulkan Validation Error: ", pCallbackData->pMessage);
    } else if (messageSeverity >= VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT) {
        INFLOG_WARN("Vulkan Validation Warning: ", pCallbackData->pMessage);
    } else if (messageSeverity >= VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT) {
        INFLOG_INFO("Vulkan Validation Info: ", pCallbackData->pMessage);
    }
...
{
    INFLOG_INFO("Initializing Vulkan device context...");

    // Step 1: Create instance
    if (!CreateInstance(config)) {
        INFLOG_ERROR("Failed to create Vulkan instance");
        return false;
    }

    // Step 2: Setup debug messenger (if validation enabled)
    if (m_validationEnabled && !SetupDebugMessenger()) {
        INFLOG_WARN("Failed to setup debug messenger");
        // Not fatal, continue
    }

    // Step 3: Create surface
    if (!CreateSurface(window)) {
        INFLOG_ERROR("Failed to create window surface");
        return false;
    }

    // Step 4: Pick physical device
    if (!PickPhysicalDevice(config)) {
        INFLOG_ERROR("Failed to find suitable GPU");
        return false;
    }

    // Step 5: Create logical device
    if (!CreateLogicalDevice(config)) {
        INFLOG_ERROR("Failed to create logical device");
        return false;
    }
    ...
    if (m_vmaAllocator == VK_NULL_HANDLE) {
        INFLOG_ERROR("Failed to create VMA allocator");
        return false;
    }

    INFLOG_INFO("Vulkan device context initialized successfully");
    INFLOG_INFO("  GPU: ", m_deviceProperties.deviceName);
    INFLOG_INFO("  API Version: ", VK_VERSION_MAJOR(m_deviceProperties.apiVersion), ".",
...
{
    INFLOG_INFO("Initializing Vulkan instance (split mode)...");
    ...
    INFLOG_INFO("Vulkan instance created successfully");
    return true;
}
...
bool VkDeviceContext::InitializeDevice(VkSurfaceKHR surface, const DeviceConfig &config)
{
    if (m_instance == VK_NULL_HANDLE) {
        INFLOG_ERROR("Instance not initialized. Call InitializeInstance first.");
        return false;
    }

    if (surface == VK_NULL_HANDLE) {
        INFLOG_ERROR("Invalid surface handle");
        return false;
    }

    INFLOG_INFO("Initializing Vulkan device with external surface...");
    ...
    INFLOG_INFO("Vulkan device initialized successfully");
    INFLOG_INFO("  GPU: ", m_deviceProperties.deviceName);
    INFLOG_INFO("  API Version: ", VK_VERSION_MAJOR(m_deviceProperties.apiVersion), ".",
                VK_VERSION_MINOR(m_deviceProperties.apiVersion), ".", VK_VERSION_PATCH(m_deviceProperties.apiVersion));
...
bool VkDeviceContext::CreateInstance(const DeviceConfig &config)
{
    ...
    if (m_validationEnabled && !CheckValidationLayerSupport()) {
        INFLOG_WARN("Validation layers requested but not available");
    ...
    if (result != VK_SUCCESS) {
        INFLOG_ERROR("vkCreateInstance failed: ", VkResultToString(result));
        return false;
    }
...
bool VkDeviceContext::CreateSurface(SDL_Window *window)
{
    if (!SDL_Vulkan_CreateSurface(window, m_instance, nullptr, &m_surface)) {
        INFLOG_ERROR("SDL_Vulkan_CreateSurface failed: ", SDL_GetError());
        return false;
    }
    return true;
}
...
bool VkDeviceContext::PickPhysicalDevice(const DeviceConfig &config)
{
    ...
    if (deviceCount == 0) {
        INFLOG_ERROR("No GPUs with Vulkan support found");
        return false;
    }
    ...
    if (bestDevice == VK_NULL_HANDLE) {
        INFLOG_ERROR("No suitable GPU found");
        return false;
    }
...
bool VkDeviceContext::CreateLogicalDevice(const DeviceConfig &config)
{
    ...
    if (result != VK_SUCCESS) {
        INFLOG_ERROR("vkCreateDevice failed: ", VkResultToString(result));
        return false;
    }
