...
// (handled globally by core/config/InfPlatform.h via InfPath.h)
...
SwapchainResult VkSwapchainManager::Present(uint32_t imageIndex)
{
    if (imageIndex >= m_renderFinishedSemaphores.size()) {
        INFLOG_ERROR("Present received invalid image index ", imageIndex, " for ", m_renderFinishedSemaphores.size(),
...
bool VkSwapchainManager::CreateRenderFinishedSemaphores()
{
    ...
    for (size_t i = 0; i < m_renderFinishedSemaphores.size(); ++i) {
        if (vkCreateSemaphore(m_device, &semaphoreInfo, nullptr, &m_renderFinishedSemaphores[i]) != VK_SUCCESS) {
            INFLOG_ERROR("Failed to create render-finished semaphore for swapchain image ", i);
            DestroyRenderFinishedSemaphores();
            return false;
        }
    }
