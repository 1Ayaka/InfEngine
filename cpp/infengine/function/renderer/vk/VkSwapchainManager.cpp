...
bool VkSwapchainManager::Create(const VkDeviceContext &context, uint32_t width, uint32_t height)
{
    ...
    if (result != VK_SUCCESS) {
        INFLOG_ERROR("Failed to create swapchain: ", VkResultToString(result));
        return false;
    }
    ...
    INFLOG_INFO("Swapchain created: ", m_extent.width, "x", m_extent.height, ", ", m_images.size(), " images, format ",
...
bool VkSwapchainManager::Recreate(const VkDeviceContext &context, uint32_t width, uint32_t height)
{
    ...
    if (swapchainSupport.capabilities.currentExtent.width == 0 ||
        ...
        INFLOG_WARN("Swapchain recreation skipped: zero extent (minimized window?)");
        return false;
    }
    ...
    VkResult result = vkCreateSwapchainKHR(m_device, &createInfo, nullptr, &m_swapchain);
    if (result != VK_SUCCESS) {
        INFLOG_ERROR("Failed to recreate swapchain: ", VkResultToString(result));
        return false;
    }
    ...
    INFLOG_INFO("Swapchain recreated: {", m_extent.width, "}, {", m_extent.height, "}, ", m_images.size(),
...
SwapchainResult VkSwapchainManager::AcquireNextImage(uint32_t &imageIndex)
{
    ...
    if (result != VK_SUCCESS) {
        INFLOG_ERROR("Failed to acquire swapchain image: ", VkResultToString(result));
...
SwapchainResult VkSwapchainManager::Present(uint32_t imageIndex)
{
    ...
    if (result != VK_SUCCESS) {
        INFLOG_ERROR("Failed to present swapchain image: ", VkResultToString(result));
        return SwapchainResult::Error;
    }
...
VkSemaphore VkSwapchainManager::GetRenderFinishedSemaphore() const
{
    return m_syncObjects[m_currentFrame].renderFinishedSemaphore;
}
...
VkPresentModeKHR VkSwapchainManager::ChoosePresentMode(const std::vector<VkPresentModeKHR> &modes) const
{
    ...
    // Fallback to FIFO (always available, V-Sync)
...
bool VkSwapchainManager::CreateImageViews()
{
    ...
    for (size_t i = 0; i < m_images.size(); i++) {
        ...
        if (result != VK_SUCCESS) {
            INFLOG_ERROR("Failed to create image view ", i, ": ", VkResultToString(result));
            return false;
        }
    }
...
bool VkSwapchainManager::CreateSyncObjects()
{
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        if (vkCreateSemaphore(m_device, &semaphoreInfo, nullptr, &m_syncObjects[i].imageAvailableSemaphore) !=
                VK_SUCCESS ||
            vkCreateSemaphore(m_device, &semaphoreInfo, nullptr, &m_syncObjects[i].renderFinishedSemaphore) !=
                VK_SUCCESS ||
            vkCreateFence(m_device, &fenceInfo, nullptr, &m_syncObjects[i].inFlightFence) != VK_SUCCESS) {
            INFLOG_ERROR("Failed to create sync objects for frame ", i);
            return false;
        }
    }
