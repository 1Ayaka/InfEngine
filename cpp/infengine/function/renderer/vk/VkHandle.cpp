                            ...
                            VkMemoryPropertyFlags properties)
{
    ...
    if (result != VK_SUCCESS) {
        INFLOG_ERROR("Failed to create buffer with VMA (VkResult: {})", static_cast<int>(result));
...
void *VkBufferHandle::Map(VkDeviceSize offset, VkDeviceSize size)
{
    ...
    if (vmaMapMemory(m_allocator, m_allocation, &m_mappedPtr) != VK_SUCCESS) {
        INFLOG_ERROR("Failed to map buffer memory via VMA");
        return nullptr;
    }
...
VkImageHandle::VkImageHandle(VkImageHandle &&other) noexcept
    : m_allocator(other.m_allocator), m_device(other.m_device), m_image(other.m_image), m_view(other.m_view),
      m_allocation(other.m_allocation), m_width(other.m_width), m_height(other.m_height), m_format(other.m_format)
{
    other.m_allocator = VK_NULL_HANDLE;
    other.m_device = VK_NULL_HANDLE;
                           ...
                           VkSampleCountFlagBits samples)
{
    ...
    if (result != VK_SUCCESS) {
        INFLOG_ERROR("Failed to create image with VMA (VkResult: {})", static_cast<int>(result));
...
bool VkImageHandle::CreateView(VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels)
{
    if (m_image == VK_NULL_HANDLE) {
        INFLOG_ERROR("Cannot create view for null image");
        return false;
    }
    ...
    if (vkCreateImageView(m_device, &viewInfo, nullptr, &m_view) != VK_SUCCESS) {
        INFLOG_ERROR("Failed to create image view");
        return false;
    }
                             ...
                             VkSamplerAddressMode addressMode)
{
    ...
    if (vkCreateSampler(device, &samplerInfo, nullptr, &m_sampler) != VK_SUCCESS) {
        INFLOG_ERROR("Failed to create sampler");
        return false;
    }
                                 ...
                                 uint32_t width, uint32_t height, VkFormat format)
{
    ...
    // Create image
    if (!m_image.Create(allocator, device, width, height, format, VK_IMAGE_TILING_OPTIMAL,
    ...
    // (Simplified - in production, use command buffer helpers)
    ...
    // Create sampler
    if (!m_sampler.Create(device, physicalDevice)) {
        return false;
    }
