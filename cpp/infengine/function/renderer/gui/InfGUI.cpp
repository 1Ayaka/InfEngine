...
InfGUI::InfGUI(InfVkCoreModular *vkCore) : m_vkCore_ptr(vkCore)
{
}
...
void InfGUI::Init(SDL_Window *window)
{
    IMGUI_CHECKVERSION();
    ...
    //
    // NOTE: Swapchain is VK_FORMAT_B8G8R8A8_SRGB, so the GPU applies
    // sRGB gamma encoding on output.  ImGui colours are specified in
    // "display" (sRGB) space but written into a framebuffer that does
    // another sRGB encode, making everything brighter.  The helper
    // lambda below converts sRGB hex → linear, so the final output
    // after the hardware sRGB encode matches the intended hex colour.
    // =========================================================================
    {
        ImGuiStyle &style = ImGui::GetStyle();
        ImVec4 *c = style.Colors;

        // sRGB → linear conversion so colours match the hex values after
        // the hardware sRGB gamma curve on the B8G8R8A8_SRGB swapchain.
        auto S = [](float srgb) -> float {
            return (srgb <= 0.04045f) ? srgb / 12.92f : powf((srgb + 0.055f) / 1.055f, 2.4f);
        };
        auto L = [&S](float r, float g, float b, float a) -> ImVec4 { return ImVec4(S(r), S(g), S(b), a); };
        ...
        c[ImGuiCol_FrameBgActive] = L(0.200f, 0.200f, 0.200f, 1.00f);  // #333333
        ...
        // --- Interactive accent ---
        c[ImGuiCol_CheckMark] = L(1.000f, 1.000f, 1.000f, 1.00f);  // white
        c[ImGuiCol_SliderGrab] = L(0.812f, 0.812f, 0.812f, 1.00f); // #CFCFCF
        c[ImGuiCol_SliderGrabActive] = L(1.000f, 1.000f, 1.000f, 1.00f);
        c[ImGuiCol_NavHighlight] = L(0.922f, 0.341f, 0.341f, 0.70f); // #EB5757

        // --- Buttons ---  subtle raised surface
        c[ImGuiCol_Button] = L(0.165f, 0.165f, 0.165f, 1.00f);        // #2A2A2A
        c[ImGuiCol_ButtonHovered] = L(0.220f, 0.220f, 0.220f, 1.00f); // #383838
        c[ImGuiCol_ButtonActive] = L(0.260f, 0.260f, 0.260f, 1.00f);  // #424242

        // --- Header ---
        c[ImGuiCol_Header] = L(0.200f, 0.200f, 0.200f, 1.00f);
        c[ImGuiCol_HeaderHovered] = L(0.165f, 0.165f, 0.165f, 1.00f);
        c[ImGuiCol_HeaderActive] = L(0.200f, 0.200f, 0.200f, 1.00f);
        ...
        // Borders — minimal, Notion barely shows borders
        ...
        // Rounding — Notion uses generous 6 px rounding
    ...
    if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &m_descriptorPool_vk) != VK_SUCCESS) {
        INFLOG_FATAL("Failed to create descriptor pool for ImGui.");
        return;
    }
    ...
    // Create a minimal compatible render pass for ImGui (swapchain format, no depth)
    {
        ...
        if (vkCreateRenderPass(device, &rpInfo, nullptr, &m_imguiRenderPass) != VK_SUCCESS) {
            INFLOG_FATAL("Failed to create ImGui render pass.");
            return;
        }
    }
    ...
    // Use InfGUI's own render pass instead of pulling from InfVkCoreModular
    initInfo.PipelineInfoMain.RenderPass = m_imguiRenderPass;
    initInfo.PipelineInfoMain.Subpass = 0;
    initInfo.PipelineInfoMain.MSAASamples = VK_SAMPLE_COUNT_1_BIT;

    if (!ImGui_ImplVulkan_Init(&initInfo)) {
        INFLOG_FATAL("Failed to initialize ImGui Vulkan implementation.");
        return;
    }
...
void InfGUI::SetGUIFont(const char *fontPath, float fontSize)
{
    ImGuiIO &io = ImGui::GetIO();
    io.Fonts->Clear();
    ...
    ImFont *font = io.Fonts->AddFontFromFileTTF(fontPath, fontSize, &fontConfig, glyphRanges);
    if (font == nullptr) {
        INFLOG_WARN("InfGUI::SetGUIFont(): Failed to load font from ", fontPath);
        return;
    }
...
void InfGUI::BuildFrame()
{
    static auto ctx = std::make_unique<InfGUIContext>();
    ...
    constexpr float kStatusBarHeight = 24.0f; // must match _HEIGHT in status_bar.py
    ...
    if (needsDefaultLayout) {
        ...
        // Split: Left panel (Hierarchy) | Center + Right
        ImGuiID dockLeft;
        ImGuiID dockRightMain;
        ImGui::DockBuilderSplitNode(dockspaceId, ImGuiDir_Left, 0.18f, &dockLeft, &dockRightMain);

        // Split right main: Center (Scene/Game) | Right (Inspector)
        ImGuiID dockCenter;
        ImGuiID dockRight;
        ImGui::DockBuilderSplitNode(dockRightMain, ImGuiDir_Right, 0.25f, &dockRight, &dockCenter);

        // Split center: Top (Scene/Game) | Bottom (Console/Project)
        ImGuiID dockCenterTop;
        ImGuiID dockCenterBottom;
        ...
        ImGui::DockBuilderDockWindow(u8"控制台 Console", dockCenterBottom);
        ImGui::DockBuilderDockWindow(u8"项目 Project", dockCenterBottom);
    ...
    // --- Per-panel profiling (prints every 120 frames) ---
#ifndef INFENGINE_GUI_PANEL_PROFILE
#define INFENGINE_GUI_PANEL_PROFILE 0
#endif
#if INFENGINE_GUI_PANEL_PROFILE
    using hrc = std::chrono::high_resolution_clock;
    static int guiProfileFrame = 0;
    static std::unordered_map<std::string, double> guiAccum;
    ++guiProfileFrame;
    for (const auto &name : m_renderableOrder) {
        ...
        auto t1 = hrc::now();
        guiAccum[name] += std::chrono::duration<double, std::milli>(t1 - t0).count();
    }
    if (guiProfileFrame >= 120) {
        std::string line = "[GUIProfile]";
        for (auto &[name, ms] : guiAccum) {
            char buf[64];
            snprintf(buf, sizeof(buf), " %s=%.2fms", name.c_str(), ms / guiProfileFrame);
            line += buf;
        }
        INFLOG_WARN(line);
        guiAccum.clear();
...
void InfGUI::RecordCommand(VkCommandBuffer cmdBuf)
{
    ImGui::Render();
    ImGui_ImplVulkan_RenderDrawData(ImGui::GetDrawData(), cmdBuf);
}

void InfGUI::Shutdown()
{
    // Clean up InfGUI-owned textures (image, imageView, sampler, memory).
...
void InfGUI::Register(const std::string &name, std::shared_ptr<InfGUIRenderable> renderable)
{
    auto existing = m_renderables_umap.find(name);
    if (existing != m_renderables_umap.end()) {
        INFLOG_WARN("InfGUI::Register(): Renderable with name '", name, "' already exists. Overwriting.");
    } else {
...
void InfGUI::Unregister(const std::string &name)
{
    auto it = m_renderables_umap.find(name);
    if (it != m_renderables_umap.end()) {
        ...
        INFLOG_WARN("InfGUI::Unregister(): Renderable with name '", name, "' does not exist.");
    }
}

uint64_t InfGUI::UploadTextureForImGui(const std::string &name, const unsigned char *pixels, int width, int height)
{
    ...
    if (result != VK_SUCCESS) {
        INFLOG_ERROR("InfGUI::UploadTextureForImGui(): Failed to create image for '", name, "'");
    ...
    if (vkCreateImageView(device, &viewInfo, nullptr, &tex.imageView) != VK_SUCCESS) {
        INFLOG_ERROR("InfGUI::UploadTextureForImGui(): Failed to create image view for '", name, "'");
    ...
    if (vkCreateSampler(device, &samplerInfo, nullptr, &tex.sampler) != VK_SUCCESS) {
        INFLOG_ERROR("InfGUI::UploadTextureForImGui(): Failed to create sampler for '", name, "'");
    ...
    if (tex.descriptorSet == VK_NULL_HANDLE) {
        INFLOG_ERROR("InfGUI::UploadTextureForImGui(): Failed to create ImGui texture descriptor for '", name, "'");
...
void InfGUI::RemoveImGuiTexture(const std::string &name)
{
    auto it = m_textures_umap.find(name);
    if (it == m_textures_umap.end()) {
        return;
...
bool InfGUI::HasImGuiTexture(const std::string &name) const
{
    return m_textures_umap.find(name) != m_textures_umap.end();
}

uint64_t InfGUI::GetImGuiTextureId(const std::string &name) const
{
    auto it = m_textures_umap.find(name);
    if (it != m_textures_umap.end()) {
