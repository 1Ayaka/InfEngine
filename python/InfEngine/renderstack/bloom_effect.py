...
class BloomEffect(FullScreenEffect):
    ...
    threshold: float = serialized_field(default=1.0, range=(0.0, 10.0))
    intensity: float = serialized_field(default=0.8, range=(0.0, 5.0))
    scatter: float = serialized_field(default=0.7, range=(0.0, 1.0))
    clamp: float = serialized_field(default=65472.0, range=(0.0, 65472.0))
    tint_r: float = serialized_field(default=1.0, range=(0.0, 1.0))
    tint_g: float = serialized_field(default=1.0, range=(0.0, 1.0))
    tint_b: float = serialized_field(default=1.0, range=(0.0, 1.0))
    max_iterations: int = serialized_field(default=5, range=(1, 8))
    ...
    def setup_passes(self, graph: "RenderGraph", bus: "ResourceBus") -> None:
        ...
        with graph.add_pass("Bloom_Prefilter") as p:
            ...
            p.fullscreen_quad(
                ...
                knee=0.5,  # Unity default soft knee
                clampMax=self.clamp,
            )
        ...
        # ---- Final pass: Composite (bloom + scene_copy → scene_color) ----
        # Reads from up[0] (bloom) and scene_copy (original scene),
        # writes to the camera target (scene_color). No self-dependency.
        with graph.add_pass("Bloom_Composite") as p:
            p.read(up_textures[0])    # bloom result
            p.read(scene_copy)         # original scene color
            p.write_color(color_handle)  # write back to backbuffer
            p.fullscreen_quad(
                "bloom_composite",
