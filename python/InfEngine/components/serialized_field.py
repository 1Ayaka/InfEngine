...
def _get_asset_db():
    try:
        from InfEngine.core.asset_ref import _get_asset_database
...
def get_raw_field_value(component: 'InfComponent', field_name: str) -> Any:
...
def _infer_field_type(python_type: Optional[Type], default: Any) -> FieldType:
    ...
    if python_type is not None:
        ...
        # InfComponent subclass → COMPONENT (e.g. ``text: UIText``)
        try:
            from .component import InfComponent as _IC
    ...
    if default is not None:
        ...
        if default_type_name in ('Material', 'InfMaterial'):
            return FieldType.MATERIAL
...
def resolve_annotation(annotation) -> Optional['FieldMetadata']:
    ...
    Used by ``InfComponent.__init_subclass__`` to support bare annotation
    ...
    # ── InfComponent subclass → ComponentRef ──
    try:
        from .component import InfComponent as _IC
...
def get_serialized_fields(component_class: Type['InfComponent']) -> Dict[str, FieldMetadata]:
    ...
    # rediscover CppProperty descriptors directly from the class hierarchy.
    if not fields:
        for cls in reversed(component_class.__mro__):
...
def component_list_field(
    ...
    **kwargs,
    ...
    Example::
        ...
        class TeamManager(InfComponent):
