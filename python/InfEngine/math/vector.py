...
These wrap the C++ ``vec2f`` / ``vec3f`` / ``vec4f`` types and add Unity-style
...
Usage::
    ...
    from InfEngine.math import vector3, vector2, vector4
...
from InfEngine.lib import vec2f, vec3f, vec4f, quatf
...
class _VecMeta(type):
    ...
    - Implements ``isinstance()`` / ``issubclass()`` checks against the
      underlying C++ type so that ``isinstance(vec3f(...), vector3)`` is True.
    """
    ...
    def __instancecheck__(cls, instance):
        """``isinstance(v, vector3)`` returns True for vec3f instances."""
...
class quaternion(metaclass=_VecMeta):
    ...
    Instance properties::
        ...
        q.euler_angles                   # as vec3f (degrees)
