...
from InfEngine.lib import InputManager as _NativeInputManager
...
class Input(metaclass=_InputMeta):
    ...
    def get_mouse_position() -> Tuple[float, float]:
        """Current mouse position as ``(x, y)`` — functional alternative."""
        mgr = _NativeInputManager.instance()
        return (mgr.mouse_position_x, mgr.mouse_position_y)
    ...
    @staticmethod
    def get_mouse_scroll_delta() -> Tuple[float, float]:
        """Scroll delta as ``(x, y)`` — functional alternative."""
        mgr = _NativeInputManager.instance()
        return (mgr.mouse_scroll_delta_x, mgr.mouse_scroll_delta_y)
    ...
    def get_axis(axis_name: str) -> float:
        ...
        if name == "horizontal":
            ...
            return mgr.mouse_delta_x
        elif name == "mouse y":
            return mgr.mouse_delta_y
    ...
    def get_input_string() -> str:
        """Characters typed this frame — functional alternative."""
        if not Input._game_focused:
            return ""
        return _NativeInputManager.instance().input_string
