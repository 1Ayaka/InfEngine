...
class ConsolePanel(ClosablePanel):
    ...
    def clear(self):
        ...
        self._cache_dirty = True
        from InfEngine.debug import DebugConsole
        DebugConsole.get_instance().clear()
    ...
    def on_render(self, ctx: InfGUIContext):
        ...
        # Handle focus request from status bar click
        if self._request_focus:
            ctx.set_next_window_focus()
            self._request_focus = False
        ctx.set_next_window_size(700, 250, Theme.COND_FIRST_USE_EVER)
    ...
    def _render_toolbar(self, ctx, log_cnt, warn_cnt, err_cnt):
        ...
        ctx.same_line()
        self._collapse = ctx.checkbox("Collapse", self._collapse)
        ctx.same_line()
        self._clear_on_play = ctx.checkbox("Clear on Play", self._clear_on_play)
        ctx.same_line()
        self._error_pause = ctx.checkbox("Error Pause", self._error_pause)
        ...
        ctx.same_line(max(win_w - filter_width, 0))
        self._show_info = ctx.checkbox(
            f"Log {log_cnt}" if log_cnt else "Log", self._show_info)

        ctx.same_line()
        ctx.push_style_color(ImGuiCol.Text, *Theme.LOG_WARNING)
        self._show_warnings = ctx.checkbox(
            f"Warn {warn_cnt}" if warn_cnt else "Warn", self._show_warnings)
        ctx.pop_style_color(1)

        ctx.same_line()
        ctx.push_style_color(ImGuiCol.Text, *Theme.LOG_ERROR)
        self._show_errors = ctx.checkbox(
            f"Error {err_cnt}" if err_cnt else "Error", self._show_errors)
        ctx.pop_style_color(1)
    ...
    def _render_row(self, ctx, idx, entry):
        ...
        # Show only the first line in the list; full text lives in the detail pane
        ...
        # Detect double-click on this row → open source file
    ...
    def _open_in_editor(filepath: str, line: int) -> None:
        ...
        from InfEngine.engine.ui.project_utils import open_in_vscode, open_file_with_system
