...
class SceneFileManager:
    ...
    @staticmethod
    def instance() -> Optional["SceneFileManager"]:
        return SceneFileManager._instance
    ...
    def open_scene(self, path: str) -> bool:
        ...
        The actual load is deferred until the user responds.
        """
        ...
        if self._dirty:
            ...
            return False
        return self._do_open_scene(path)
    ...
    def _do_open_scene(self, path: str) -> bool:
        ...
        if not scene.load_from_file(path):
    ...
    def _do_save(self, path: str) -> bool:
        ...
        if not scene.save_to_file(path):
            Debug.log_error(f"Failed to save scene to: {path}")
            return False
    ...
    def _save_camera_state(self, scene_path: str):
        ...
        if not self._engine or not scene_path:
            return
        pos = self._engine.get_editor_camera_position()
        rot = self._engine.get_editor_camera_rotation()
        fp = self._engine.get_editor_camera_focus_point()
        fd = self._engine.get_editor_camera_focus_distance()
        state = {
            "position": list(pos),
            "focusPoint": list(fp),
            "focusDistance": fd,
    ...
    def _restore_camera_state(self, scene_path: str):
        ...
        p = state["position"]
        f = state["focusPoint"]
        self._engine.restore_editor_camera_state(
            p[0], p[1], p[2],
            f[0], f[1], f[2],
            state["focusDistance"],
            state["yaw"],
    ...
    def _restore_py_components(self, scene):
        ...
        # Clear the active-instances registry before restoring so stale entries
        # from any previously-loaded scene don't accumulate.
        ...
        for pc in pending:
            ...
            if instance is None:
                Debug.log_warning(f"Failed to create component '{pc.type_name}'")
                continue
        ...
        Debug.log_internal(f"Restored {len(pending)} Python component(s) from scene file")
