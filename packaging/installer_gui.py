...
def _payload_dir() -> str:
    ...
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "dist", "InfEngine Hub")


_UNINSTALL_REG_KEY = r"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\InfEngineHub"


def _write_registry(install_dir: str) -> None:
    """Register InfEngine Hub in Windows Add/Remove Programs."""
    if sys.platform != "win32":
        return
    exe_path = os.path.join(install_dir, "InfEngine Hub.exe")
    uninstall_cmd = f'"{os.path.join(install_dir, "InfEngine Hub.exe")}" --uninstall'
    icon_path = os.path.join(install_dir, "InfEngineHubData", "runtime", "icon.png")
    if not os.path.isfile(icon_path):
        icon_path = exe_path

    try:
        with winreg.CreateKeyEx(winreg.HKEY_CURRENT_USER, _UNINSTALL_REG_KEY, 0, winreg.KEY_WRITE) as key:
            winreg.SetValueEx(key, "DisplayName", 0, winreg.REG_SZ, "InfEngine Hub")
            winreg.SetValueEx(key, "UninstallString", 0, winreg.REG_SZ, uninstall_cmd)
            winreg.SetValueEx(key, "DisplayIcon", 0, winreg.REG_SZ, icon_path)
            winreg.SetValueEx(key, "InstallLocation", 0, winreg.REG_SZ, install_dir)
            winreg.SetValueEx(key, "Publisher", 0, winreg.REG_SZ, "InfEngine")
...
def _remove_registry() -> None:
    """Remove InfEngine Hub from Windows Add/Remove Programs."""
    if sys.platform != "win32":
        return
    try:
...
def _create_start_menu_shortcut(install_dir: str) -> None:
    """Create a Start Menu shortcut for InfEngine Hub."""
    if sys.platform != "win32":
        return
    try:
        ...
        shortcut_dir = os.path.join(programs_dir, "InfEngine Hub")
        os.makedirs(shortcut_dir, exist_ok=True)
        shortcut_path = os.path.join(shortcut_dir, "InfEngine Hub.lnk")
        exe_path = os.path.join(install_dir, "InfEngine Hub.exe")
...
def _remove_start_menu_shortcut() -> None:
    """Remove Start Menu shortcut for InfEngine Hub."""
    if sys.platform != "win32":
        return
    try:
        import ctypes.wintypes
...
class InstallWorker(QObject):
    ...
    def run(self) -> None:
        try:
            ...
            self.progress.emit("Copying InfEngine Hub files...")
            ...
            self.progress.emit("Registering InfEngine Hub...")
...
class InstallerWindow(QWidget):
    def __init__(self):
        ...
        default_dir = os.path.join(os.environ.get("ProgramFiles", "C:\\Program Files"), "InfEngine Hub")

        root = QVBoxLayout(self)
        root.setContentsMargins(20, 20, 20, 20)
        root.setSpacing(14)

        title = QLabel("Install InfEngine Hub")
    ...
    def _launch_hub(self) -> None:
        if not self._installed_dir:
            return
        exe_path = os.path.join(self._installed_dir, "InfEngine Hub.exe")
