# InfEngine Scripting API

<div class="class-info">
Version 0.1
 &nbsp;|&nbsp; <a href="../../zh/api/index.html">中文</a>
</div>

## Description

Welcome to the InfEngine Scripting API Reference. Browse packages from the sidebar to see class documentation.

<!-- USER CONTENT START --> description

<!-- USER CONTENT END -->

## Packages

| Package | Description |
|------|------|
| InfEngine | LogLevel, PrimitiveType, LightType, LightShadows, Component, Transform, ... |
| InfEngine.components | InfComponent, require_component, disallow_multiple, execute_in_edit_mode |
| InfEngine.components.builtin | Camera, Light, MeshRenderer |
| InfEngine.core | Material, MeshData, Shader, Texture |
| InfEngine.debug | Debug |
| InfEngine.gizmos | Gizmos |
| InfEngine.input | KeyCode, Input |
| InfEngine.math | vector3, vector2, vector4 |
| InfEngine.scene | LayerMask, SceneManager |

<!-- USER CONTENT START --> index

<!-- USER CONTENT END -->
